...
{
	[CustomEditor(typeof(RuleTile2DKit))]
	...
	internal class RuleTileEditor : Editor
	{
		...
		public RuleTile2DKit tile { get { return (target as RuleTile2DKit); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
				tile.m_TilingRules = new List<RuleTile2DKit.TilingRule>();

			m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(RuleTile2DKit.TilingRule), true, true, true, true);
		...
		{
			RuleTile2DKit.TilingRule rule = tile.m_TilingRules[index];
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTile2DKit.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case RuleTile2DKit.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case RuleTile2DKit.TilingRule.Neighbor.NotThis:
								GUI.DrawTexture(r, arrows[9]);
								break;
						}
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_Neighbors[index] = (RuleTile2DKit.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + 1) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case RuleTile2DKit.TilingRule.AutoTransform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case RuleTile2DKit.TilingRule.AutoTransform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case RuleTile2DKit.TilingRule.AutoTransform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_AutoTransform = (RuleTile2DKit.TilingRule.AutoTransform)(((int)tilingRule.m_AutoTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public RuleTile2DKit.TilingRule m_Rule;
			public RuleTile2DKit.TilingRule.AutoTransform m_NewValue;

			public MenuItemData(RuleTile2DKit.TilingRule mRule, RuleTile2DKit.TilingRule.AutoTransform mNewValue)
			{
				this.m_Rule = mRule;
		...
		private void SpriteOnGUI(Rect rect, RuleTile2DKit.TilingRule tilingRule)
		{
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTile2DKit.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_AutoTransform = (RuleTile2DKit.TilingRule.AutoTransform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_AutoTransform);
			...
			tilingRule.m_Output = (RuleTile2DKit.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == RuleTile2DKit.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == RuleTile2DKit.TilingRule.OutputSprite.Random)
			{
			...
			if (tilingRule.m_Output != RuleTile2DKit.TilingRule.OutputSprite.Single)
			{
